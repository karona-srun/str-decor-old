    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-outline card-primary">
                ...
                <div class="card-body">
                    <form id="quickForm" action="{{ url('expend', $expend->id) }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <div class="card-body">
                            <div class="row">
                                        ...
                                        <select class="form-control select2" name="expend_option" style="width: 100%;">
                                            ...
                                            @foreach ($expend_options as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $item->id == $expend->expend_option_id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="date" name="date" id="date" class="form-control"
                                            value="{{ $expend->date }}" placeholder="{{ __('app.table_date') }}">
                                        @if ($errors->has('date'))
                                            <div class="error text-danger text-sm mt-1">
